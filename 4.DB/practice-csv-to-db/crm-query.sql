...
select storeid, storename, max(totalprice)
from (select S.uuid as storeid, S.name as storename, sum(I.unitprice) as totalprice
from store S
join orders O on O.storeid = S.uuid
join orderitem OI on O.uuid = OI.orderid
join item I on OI.itemid = I.uuid
...
select userid, username, max(TotalSales)
from (select U.uuid as userid, U.name as username, sum(I.unitprice) as TotalSales
from user U
join orders O on O.userid = U.uuid
join store S on S.uuid = O.storeid
join orderitem OI on OI.orderid = O.uuid
